       subroutine model_d(x,y,lx,ly,matrix)
       ...
       real, intent(inout) :: x(lx*ly*3), y(lx*ly*2), matrix(2,3)
       real vec(3), vec2(2)
       !Counters
       ...
       do i=1,ly
        do k=1,lx
         ...
         do cont=1,2
          do cont2=1,3
           vec2(cont)=vec2(cont) +matrix(cont,cont2)*vec(cont2)
          enddo
         enddo
         y(k    + ((lx*2)*(i-1)) ) = vec2( 1  )
       ...
       subroutine model_c(x,y,lx,ly,matrix)
       ...
       integer, intent(inout) :: LX, LY
       !Vectors to be convolved
       real, intent(inout) :: x(lx*ly*2), y(lx*ly*3), matrix(2,3)
       real vec(3), vec2(2)
       !Counters
       integer i, k, cont, cont2
       ...
       do i=1,ly
        do k=1,lx
         ...
         do cont=1,3
          do cont2=1,2
           vec(cont)=vec(cont) +matrix(cont2,cont)*vec2(cont2)
          enddo
         enddo
         y(k     + ((lx*3)*(i-1)) ) = vec( 1 )
